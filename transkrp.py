...
def video_id(url: str) -> str | None:
    ...
    """
    if ep := podcast.episode_id(url):
        return ep  # a podcast episode's id, which serves the same purpose
...
def expand(url: str, proxy: str | None = None, cookies: str | None = None,
    ...
    if podcast.is_podcast(url):
        return _expand_podcast(url, force_playlist)
...
def _expand_podcast(url: str, force_playlist: bool) -> list[str]:
    """A feed is a playlist, but an expensive one, so it doesn't expand by default.

    A YouTube playlist costs a caption fetch per video. A feed costs a *whisper
    run* per episode — 21 episodes of the show this was built against is about
    fourteen hours of CPU. Expanding that because someone pasted a show name is
    not a default, it's a trap, so a show means its latest episode unless
    --playlist says otherwise. Which reading was used is printed either way.
    """
    if url.startswith(podcast.REF):
        return [url]  # already one episode, from a previous expansion
    if not force_playlist:
        return [url]
    feed = podcast.feed_url(url)
    show = podcast.episodes(feed)
    out = [podcast.ref(feed, e["id"]) for e in show["episodes"]
           if e["audio"] or e["transcript_url"]]
    if not out:
        raise LookupError(f"{show['show'] or url!r} has no episodes with audio")
    return out
...
def transcript(url: str, lang: str | None = None, proxy: str | None = None,
    ...
    Returns a JSON-safe dict:
        title, video_id, url, source ("manual"|"auto"|"whisper"|"published"),
        lang, translated, duration_ms,
        paragraphs: [{start_ms, timestamp, turn, text}], text

    Takes a YouTube URL, an RSS feed, an Apple Podcasts link, or a show's name —
    a podcast goes down a different road (no captions exist, so the audio is
    transcribed) and comes back in the same shape.
...
def to_markdown(t: dict) -> str:
    ...
    if t.get("feed"):
        head.append(f"feed: {t['feed']}")
    head += [f"url: {t['url']}"]
    # The audio is what the timestamps resolve to, so the document has to carry
    # it: the episode page can be redesigned or lost, the enclosure is the thing
    # actually cited.
    if t.get("audio"):
        head.append(f"audio: {t['audio']}")
    head.append(f"source: {t['source']}"
                + ("  # generated by speech recognition, not a published transcript"
    ...
    elif t["source"] == "whisper":
        # Whisper punctuates, so the unpunctuated warning above never fires for
        # it — and its actual weakness is the one a corpus of people cares about
        # most. It transcribes conversational English well and proper nouns
        # badly; every name in this file is a guess with a timestamp beside it.
        head.append("note: no transcript is published for this episode; every word "
                    "here is ASR output and names in particular should be checked "
                    "against the audio")
    ...
    for p in t["paragraphs"]:
        ...
        anchor = _at(t.get("audio") or t["url"], p["start_ms"])
        lines.append(f"[{p['timestamp']}]({anchor}) {marker}{p['text']}")
        prev_turn = p["turn"]
    return "\n".join(head) + f"\n\n# {t['title']}\n\n" + "\n\n".join(lines) + "\n"


def _at(url: str, ms: int) -> str:
    """The same recording, at that second.
    ...
    An episode MP3 gets `#t=`, the media-fragment form every browser honours on
    an audio file. Without it a podcast document's timestamps all linked to the
    top of the same hour-long file, which is a citation that doesn't resolve —
    the one thing every other decision here is against.
    """
    if m := _VIDEO_URL.search(url):
        return f"https://www.youtube.com/watch?v={m.group(0)[-11:]}&t={ms // 1000}s"
    base = url.split("#")[0]
    return f"{base}#t={ms // 1000}" if base else base
...
def main(argv: list[str] | None = None) -> int:
    ...
    ap.add_argument("--playlist", action="store_true",
                    help="for a URL that names a video inside a playlist, take "
                         ...
                         "podcast, take every episode rather than the latest")
    ...
    # Same courtesy as the ambiguous-playlist note above, and the default here is
    # far more consequential: every episode is a whisper run, so a show takes one
    # and says so rather than silently starting fourteen hours of work.
    if not args.playlist and not args.episode:
        for u in args.url:
            if podcast.is_podcast(u) and not u.startswith(podcast.REF):
                print(f"note: {u} names a show; taking its most recent episode. "
                      f"--episode TITLE picks another, --playlist takes them all.",
                      file=sys.stderr)
...
def _list_episodes(url: str) -> int:
    """What a feed has, so --episode can name one of them."""
    try:
        show = podcast.episodes(podcast.feed_url(url))
    except LookupError as e:
        print(f"error: {url}: {e}", file=sys.stderr)
        return 1
    print(show["show"])
    ...
    for ep in show["episodes"]:
        mins = f"{ep['duration_s'] // 60}m" if ep["duration_s"] else "?"
        # Say which ones are free. A published transcript skips whisper
        # entirely, and that is the difference between seconds and minutes.
        how = "published transcript" if ep["transcript_url"] else "whisper"
        print(f"  {podcast._rfc822(ep['published']) or '?':10}  {mins:>5}  "
              f"[{how}]  {ep['title']}")
    return 0
